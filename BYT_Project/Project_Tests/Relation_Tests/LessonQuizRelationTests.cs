...
namespace Project_Tests.Relation_Tests
{
    ...
    public class LessonQuizRelationTests
    {
        ...
        [Test]
        public void TestErrorWhenAssigningNullToLesson()
        {
            var quiz = new Quiz(1, "Math Quiz", 100, 60);

            var ex = Assert.Throws<ArgumentNullException>(() => quiz.AssignToLesson(null));
            Assert.That(ex.ParamName, Is.EqualTo("lesson"), "Assigning null to quiz lesson did not throw expected exception.");
        }

        [Test]
        public void TestErrorWhenAddingNullQuizToLesson()
        {
            var lesson = new Lesson(1, "Math Lesson", "http://example.com", "Basic math lesson");

            var ex = Assert.Throws<ArgumentNullException>(() => lesson.AddQuiz(null));
            Assert.That(ex.ParamName, Is.EqualTo("quiz"), "Adding null quiz to lesson did not throw expected exception.");
        }

        [Test]
        public void TestQuizReassignmentWithoutExplicitRemoval()
        {
            var lesson1 = new Lesson(1, "Math Lesson 1", "http://example.com/1", "Basic math lesson 1");
            var lesson2 = new Lesson(2, "Math Lesson 2", "http://example.com/2", "Basic math lesson 2");
            var quiz = new Quiz(1, "Math Quiz", 100, 60);
            ...
            // Explicitly remove from the old lesson
            lesson1.RemoveQuiz(quiz);
            lesson2.AddQuiz(quiz);
        ...
        [Test]
        public void TestAddingQuizToNewLessonDirectlyHandlesPreviousAssociation()
        {
            var lesson1 = new Lesson(1, "Math Lesson 1", "http://example.com/1", "Basic math lesson 1");
            var lesson2 = new Lesson(2, "Math Lesson 2", "http://example.com/2", "Basic math lesson 2");
            var quiz = new Quiz(1, "Math Quiz", 100, 60);

            // Add quiz to the first lesson
            lesson1.AddQuiz(quiz);

            // Reassign directly to the second lesson using AssignToLesson
            quiz.AssignToLesson(lesson2);

            Assert.Multiple(() =>
            {
                Assert.That(!lesson1.Quizzes.Contains(quiz), "Quiz still associated with the old lesson after reassignment via AssignToLesson.");
                Assert.That(lesson2.Quizzes.Contains(quiz), "Quiz not associated with the new lesson after reassignment via AssignToLesson.");
                Assert.That(quiz.Lesson, Is.EqualTo(lesson2), "Quiz lesson reference not updated correctly after reassignment via AssignToLesson.");
            });
        }
    }
}
